...
export default function Sidebar({ user, canLog, periodEnd, experiments, setups = [], lang, onSignOut, onNewSession, onToggleLang }) {
  ...
  const inner = (
    <div className="flex flex-col h-full bg-slate-950">
      ...
      <div className="flex-1 overflow-y-auto py-4 px-3 space-y-6">
        ...
        {/* ── TRADE JOURNAL ── */}
        <div>
          <p className="px-2 mb-2 text-xs text-slate-300 uppercase tracking-widest font-semibold">
          ...
          <button
            ...
            className="w-full flex items-center gap-2 px-2 py-2 rounded-lg text-sm text-slate-400 hover:bg-slate-800 hover:text-white transition-colors text-left"
          >
            <span className="text-slate-600 text-xs">≡</span>
            {pt ? "Todas as operações" : "All trades"}
          </button>

          {/* Reports */}
          <button
            onClick={() => { router.push("/journal/reports"); setSidebarOpen(false); }}
            className="w-full flex items-center gap-2 px-2 py-2 rounded-lg text-sm text-slate-400 hover:bg-slate-800 hover:text-white transition-colors text-left"
          >
            <span className="text-slate-600 text-xs">◈</span>
          ...
          {setups.length > 0 && (
            <div className="mt-2">
              <p className="px-2 py-1 text-[10px] text-slate-600 uppercase tracking-widest">
                {pt ? "Setups" : "Setups"} · {setups.length}/10
              </p>
              {setups.map((setup) => (
                <button
                  key={setup.id}
                  onClick={() => { router.push(`/journal/setups/${setup.id}`); setSidebarOpen(false); }}
                  className="w-full flex items-center gap-2 px-2 py-1.5 rounded-lg text-sm text-slate-400 hover:bg-slate-800 hover:text-white transition-colors text-left"
                >
              ...
              {setups.length < 10 && (
                <button
                  onClick={() => { router.push("/journal/setups/new"); setSidebarOpen(false); }}
                  className="w-full flex items-center gap-2 px-2 py-1.5 rounded-lg text-xs text-slate-600 hover:text-slate-400 transition-colors text-left"
                >
                  <span>+ {pt ? "Novo setup" : "New setup"}</span>
                </button>
              )}
            </div>
          )}
          {setups.length === 0 && (
            <button
              ...
              className="w-full flex items-center gap-2 px-2 py-1.5 rounded-lg text-xs text-slate-600 hover:text-slate-400 transition-colors text-left"
            >
              <span>+ {pt ? "Criar primeiro setup" : "Create first setup"}</span>
            </button>
          )}
        </div>
