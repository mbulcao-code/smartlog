...
export function getNextQuestion(painType, behavior) {
  switch (painType) {
    ...
    case "other":
      ...
      if (!("has_tested_setup" in behavior)) {
        return {
          key: "has_tested_setup",
          question: {
            en: "Do you have a tested, profitable setup for this type of trade?",
            pt: "Você tem um setup testado e lucrativo para esse tipo de operação?",
          },
          options: [
            { value: "no",          en: "No — need to work on that ASAP",  pt: "Não — preciso trabalhar nisso urgente" },
            { value: "yes_used",    en: "Yes — and I used it",             pt: "Sim — e usei"                          },
            { value: "yes_not_used",en: "Yes — but I didn't use it",       pt: "Sim — mas não usei"                    },
          ],
        };
      }
      return null;
...
export function getNextAfterTradeQuestion(afterTrade) {
  ...
  if (!("setup_type" in afterTrade)) {
    return {
      key: "setup_type",
      question: {
        en: "How would you classify this trade?",
        pt: "Como você classificaria essa operação?",
      },
      options: [
